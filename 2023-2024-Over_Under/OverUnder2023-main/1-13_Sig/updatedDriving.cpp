...
int numTimes = 0;
double perShot = 360.0;
bool intakePneuPosition = false;
bool wingPneuPosition = false;
bool autonIsNotRunning = false;
void shootCatapult() {
  isShooting = false;
  while (fmod(Catapult.position(degrees), perShot) > 300) {
    Catapult.spin(forward);
    if (controller(primary).ButtonB.pressing()) {
...
void doMatchloads() {
  x=100-x;
  Catapult.spin(forward);
  Catapult.setVelocity(x, percent);
}
void stopMatchloads() {
  y=100-y;
  Catapult.spin(forward);
  Catapult.setVelocity(y, percent);
}
...
void usercontrol(void) {
    ...
    controller(primary).ButtonR1.pressed(toggleIntake);
    controller(primary).ButtonR2.pressed(toggleWings);
    ...
    controller(primary).ButtonRight.pressed(extension);
